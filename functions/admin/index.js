...
<meta name="version" content="v1.0.51">
<style>
...
<div id="mainPanel" style="display:none">
  ...
  <div class="sub">v1.0.51 · LINE Bot 統一維護</div>
...
async function openTaskDetail(id) {
  ...
  // v1.0.51: 進行中 + 菜單模式 → 顯示「按菜單重算」按鈕，依 menu_json 重設所有 entries.price
...
// v1.0.51 按菜單重算金額：呼叫 POST /api/admin/tasks/<id>/reprice
async function reprice(id) {
  ...
  if (!r.ok) {
    ...
    alert('重算失敗：' + t.slice(0, 200));
    return;
  }
  const j = await r.json();
  alert(\`重算完成\\n  共 \${j.total} 筆\\n  更新 \${j.updated} 筆\\n  原本就對 \${j.unchanged} 筆\\n  菜單外品項略過 \${j.skipped_not_in_menu} 筆\\n  沒品項略過 \${j.skipped_no_item} 筆\`);
  // 重新載入詳情顯示新 price
  openTaskDetail(id);
}
...
async function loadGroupOptionsForMembers() {
  ...
  // v1.0.51: 存 metadata，給 renderAllMembers 判斷該群是否分區
  GROUPS_META = {};
...
function renderAllMembers() {
  ...
  // v1.0.51: per-group 模式 + 該群 show_zones=0 → 分區欄改顯示「—（不分區群組）」
