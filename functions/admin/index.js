...
<meta name="version" content="v1.0.60">
<style>
...
<div id="mainPanel" style="display:none">
  ...
  <div class="sub">v1.0.60 · LINE Bot 統一維護</div>
...
async function openTaskDetail(id) {
  ...
  // v1.0.60: 進行中 + 菜單模式 → 顯示「按菜單重算」按鈕，依 menu_json 重設所有 entries.price
...
// v1.0.60 按菜單重算金額：呼叫 POST /api/admin/tasks/<id>/reprice
...
async function loadGroupOptionsForMembers() {
  ...
  // v1.0.60: 存 metadata，給 renderAllMembers 判斷該群是否分區
  GROUPS_META = {};
...
function renderAllMembers() {
  ...
  // v1.0.60: per-group 模式 + 該群 show_zones=0 → 分區欄改顯示「—（不分區群組）」
