...
錯字自動糾正（要有把握才改）：
- 常見輸入法/諧音錯字用常識糾正，例：「機腿飯」→「雞腿飯」、「排骨飲」→「排骨飯」、「珍奶」保留（是正常簡稱）、「拿鉄」→「拿鐵」、「美是」→「美式」、「冬瓜檸樣」→「冬瓜檸檬」
- 糾正後把「糾正後的標準名」填入品項，原始文字保留在 raw_text 不動
- 不確定就不要亂改
...
回傳 dup_intent + dup_confidence（0~100 整數，對此判斷的把握）。` : 'dup_intent 固定為 null、dup_confidence 為 null。'}

嚴格回傳 JSON：
{"data":{...僅本次抽到的欄位，不要自行合併舊資料...},"note":null|string,"price":null|number,"missing":string[],"follow_up":null|string,"confidence":"high"|"mid"|"low","nonsense":boolean,"dup_intent":"replace"|"add"|"unclear"|null,"dup_confidence":number|null}`;
  const r = await fetch(`${ENDPOINT}?key=${apiKey}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
